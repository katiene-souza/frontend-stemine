    ...
    >
      <Box sx={{ padding: '16px', display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2 }}>
          ...
          <Box sx={{ flexGrow: 1 }}>
            <Typography variant="h6" sx={{ fontWeight: 'bold', color: COLORS_APP.text.primary, lineHeight: 1.2 }}>
              {name}
            </Typography>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
              <Typography variant="body2" sx={{ color: COLORS_APP.text.secondary, mr: 1 }}>
                ⭐ {rating} ({sessionsCount} {MENTORING_PAGE_CONTENT.mentor_card.sessions_completed})
              </Typography>
              <Typography variant="body2" sx={{ color: COLORS_APP.text.secondary }}>
                {experienceYears}
              </Typography>
        ...
        {/* Especialidades */}
        <Typography variant="subtitle2" sx={{ fontWeight: 'bold', color: COLORS_APP.text.primary, mb: 1 }}>
          {MENTORING_PAGE_CONTENT.mentor_card.specialties_mentor}
        </Typography>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', mb: 2 }}>
          {specialties.map((spec, index) => (
            <SpecialtyTag key={index} text={spec} color={COLORS_APP.brand_colors.stemine_purple} />
          ))}
        </Box>

        {/* Descrição */}
        <Typography variant="body2" sx={{ color: COLORS_APP.text.secondary, mb: 2, flexGrow: 1, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
          Descrição: {description}
        </Typography>
        
        {/* Slots de Mentoria (Visível para Admin para gestão) */}
        {isAdm && (
          <Typography variant="body2" sx={{ color: COLORS_APP.brand_colors.stemine_purple, fontWeight: 'bold', mb: 2 }}>
            {mentoringSlots} {MENTORING_PAGE_CONTENT.availability_info.mentoring_quantity_label}
          </Typography>
        )}
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 'auto' }}>
          ...
          <Button
            ...
            sx={{
              ...
              fontWeight: 'bold',
              borderRadius: '4px',
              '&:hover': { backgroundColor: COLORS_APP.brand_colors.stemine_purple_light },
            }}
          >
